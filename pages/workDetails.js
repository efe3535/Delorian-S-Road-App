...
const WorkDetails = ({ navigation, route }) => {
    ...
    const renderItem = ({ item, index }) => {
        return (
            <SafeAreaView style={{ marginTop: 15, marginLeft: 30 }}>
                <TouchableOpacity onPress={() => { setCurrentPhoto(item.photo) ; setShowImageFullscreen(true) }} style={{ margin: 10.5 }}>
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: isDark ? "#1b1b1b" : "#fff" }}>
            <Modal visible={showImageFullscreen} transparent style={{ alignItems: "center", justifyContent: "center" }}>
                    <View style={{backgroundColor:"#121212ee", flex:1}}>
                        <TouchableOpacity style={{ flexDirection: "row", marginLeft: 30, marginTop: 36 }} onPress={() => { setShowImageFullscreen(false) }}>
                        ...
                        <View style={{ alignItems: "center", justifyContent: "center", flex: 1 }}>
                            <Image style={{ width: "85%", height: "85%", resizeMode: "stretch", borderRadius: 12, alignSelf: "center", justifyContent: "center" }} source={{ uri: `data:image/png;base64,${currentPhoto}`, cache: "only-if-cached", }} />
                        </View>
                    </View>
            </Modal>
            ...
            <ScrollView overScrollMode='never' style={{ backgroundColor: isDark ? "#1b1b1b" : "#fff", flex: 1 }} fadingEdgeLength={160}>
                ...
                <WebView androidHardwareAccelerationDisabled androidLayerType='software' renderToHardwareTextureAndroid={true} onLoad={() => {
                    mapRef.current.injectJavaScript(`   var marker = L.marker([${item.koorX}, ${item.koorY}],{icon:greenIcon}).addTo(mymap)
                                                        ;    mymap.setView([${item.koorX}, ${item.koorY}], 18) ; true
                        `)
